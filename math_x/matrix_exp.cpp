...
// Умножение двух квадратных матриц размера k x k
Matrix mat_mult(const Matrix& A, const Matrix& B, int k) {
...
// Быстрое возведение квадратной матрицы A в степень p по модулю MOD
Matrix mat_pow(Matrix A, ll p, int k) {
    Matrix B(k, vector<ll>(k));
    // Инициализируем res как единичную матрицу размером k×k.
    // Единичная матрица — это матрица, у которой все элементы на главной диагонали равны 1, а остальные — 0.
    // Она играет ту же роль, что и число 1 в обычном умножении чисел: A * I = I * A = A для любой квадратной матрицы A.
    // Поэтому при возведении матрицы A в степень p (т.е. A^p) через бинарную экспоненту,
    // мы начинаем с res = I как нейтрального элемента, чтобы не исказить результат в процессе умножений.
    // Это позволяет пошагово накапливать результат, корректно обрабатывая степень по аналогии с обычным fast exponentiation.
