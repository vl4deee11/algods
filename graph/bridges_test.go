...
func dfsBridges(v int, p int) {
	...
	for i := range g[v] {
		...
		if u[nv] {
			...
			// Мы говорим тут что только после посещения вершины v мы можем попасть в вершину nv,
			// если бы мы могли попасть в вершину nv по обратному ребру, то время minTime[nv] < enterTime[v]
